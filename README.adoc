...
fcmaes extends https://docs.scipy.org/doc/scipy/reference/optimize.html[scipy optimize] with additional optimization methods, fast {cpp}/Eigen-based implementations, and a coordinated parallel retry mechanism.
It supports multi-threaded use of several gradient-free
https://dietmarwo.github.io/fast-cma-es/fcmaes_documentation.html[algorithms] partly implemented in https://dietmarwo.github.io/fast-cma-es/cpp_documentation.html[{cpp}].
It is designed for real-world problems, including multi-objective and constrained optimization. The main algorithms are available in both Python and C++. They support parallel fitness evaluation as well as parallel retry strategies.

There are many practical https://github.com/dietmarwo/fast-cma-es/blob/master/tutorials/README.adoc[tutorials] and https://dietmarwo.github.io/fast-cma-es/examples_documentation.html[examples] that show how to use fcmaes in detail.

Detailed benchmark results are available in https://github.com/dietmarwo/fast-cma-es/blob/master/tutorials/Performance.adoc[performance].
...
- Includes the multi-objective and constrained optimizer MODE. It combines ideas from Differential Evolution and NSGA-II and supports parallel function evaluation. It also uses https://www.jstage.jst.go.jp/article/tjpnsec/11/2/11_18/_article/-char/en/[enhanced multiple constraint ranking], which improves constraint handling in engineering design problems.
...
=== Update: Native Rust implementation of fcmaes
See https://github.com/dietmarwo/fcmaes-rust[fcmaes-rust]. This repo is an alternative both for Rust and Python users as it provides a PyO3 API. See https://github.com/dietmarwo/fcmaes-rust/blob/main/examples/python/test_cma.py[python example].

=== Update (2.0.2): Ask Tell interface for BiteOpt
...
This is most useful for expensive objective functions where wall-clock time is dominated by the simulation or model evaluation.
In that setting, evaluating several candidates concurrently can reduce elapsed optimization time even if the optimizer itself needs a few more function evaluations.
...
=== Update: Split-Brain Automated Design of Stochastic Biochemical Circuits

https://github.com/dietmarwo/autoresearch-circuit[autoresearch-circuit] applies the same split-brain pattern to biochemical circuit design, inspired by the https://www.biorxiv.org/content/10.1101/2025.01.27.635147v1[CircuiTree] paper. An LLM proposes circuit topology — which genes regulate which. fcmaes tunes the kinetic rates against a noisy stochastic simulator. A validation stage stress-tests the result with knockouts, knockdowns, and parameter jitter. Structure and numbers stay separated, each handled by the right tool.

=== Update: The Split-Brain Pattern For Agentic Search
https://github.com/dietmarwo/autoresearch-trading[autoresearch-trading] applies Karpathys https://github.com/karpathy/autoresearch[autoresearch]-idea to trading algorithm optimization.
LLMs are good at structure. Classical optimizers are good at searching numbers.
So I built a system that separates the two. I think this pattern goes beyond trading.
Anywhere you have a simulator and a mix of logic + tunable numbers, this approach may be useful.
...
- New dependency: loguru. `numba` is optional and mainly used as a speedup for MAP-Elites and some examples.
...
Implemented both in https://github.com/dietmarwo/fast-cma-es/blob/master/fcmaes/mode.py[Python] and in https://github.com/dietmarwo/fast-cma-es/blob/master/_fcmaescpp/modeoptimizer.cpp[{cpp}]. Provides an ask/tell interface and supports constraints and parallel function evaluation.
...
- BiteOpt algorithm from Aleksey Vaneev https://github.com/avaneev/biteopt[BiteOpt]. Only a {cpp} version is provided. If your problem is single objective and if you have no clue what algorithm to apply, try this first. Works well with almost all problems. For constraints you have to use weighted penalties.

- Differential Evolution: Implemented both in https://github.com/dietmarwo/fast-cma-es/blob/master/fcmaes/de.py[Python] and in https://github.com/dietmarwo/fast-cma-es/blob/master/_fcmaescpp/deoptimizer.cpp[{cpp}]. Additional concepts implemented are
...
To get optimal scaling from parallel retry and parallel function evaluation you may still use:
...
The nanobind-based native backend now supports macOS as part of the wheel build pipeline.
If no matching wheel is available for your Python version or architecture, `pip`
...
Check the https://github.com/dietmarwo/fast-cma-es/blob/master/tutorials/README.adoc[tutorials] for more details.
...
- A manual GitHub Actions run with `publish_target=testpypi` publishes to TestPyPI.
