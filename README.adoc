...
PYGMO/PAGMO has direct support of constraints and multiple objectives where with fcmaes you have to use penalty functions. Exchange of information between parallel PAGMO threads is based on exchanging population members, which doesn't fit well with CMA-ES which recreates its whole population each generation.  
