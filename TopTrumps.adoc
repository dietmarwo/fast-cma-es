...
Note that this result shows a repeated "pattern" which means we could try to reduce the problem to a 4-dimensional one repeating the 4 arguments over the whole solution vector. May be the "freedom" given by allowing 128 different arguments is not needed for an optimal solution. In this case this single-objective TopTrump problem variant is questionable as a benchmark problem. But maybe someone comes up with a superior heterogeneous solution?
