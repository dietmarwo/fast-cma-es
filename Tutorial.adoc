...
Several real world optimization tasks are made callable via Python in fcmaes: 
...
fcames adds a python interface in https://github.com/dietmarwo/fast-cma-es/blob/master/fcmaes/astro.py[astro.py]. 
...
`Cma_cpp(2000)`, the C++ variant instead.
...
The Python CMA implementation can easily be replaced by the C++ one:
