...
GuessArg = Optional[np.ndarray | Tuple[np.ndarray, np.ndarray]]
...
class store():
    ...
    def __init__(self, dim: int, nobj: int, capacity: int = mp.cpu_count()*512) -> None:
        self.dim = dim
        self.nobj = nobj
    ...
    def add_result(self, x: ArrayLike, y: ArrayLike) -> None:
        with self.add_mutex:
    ...
    def add_results(self, xs: ArrayLike, ys: ArrayLike) -> None:
        with self.add_mutex:
            self.num_added.value += 1
            i = self.num_stored.value
    ...
    def get_front(self, update: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def get_content(self) -> Tuple[np.ndarray, np.ndarray]:
        stored = self.num_stored.value
...
class MODE(object):
    ...
    def set_guess(self, guess: GuessArg, mofun: MultiObjective, rg: Optional[Generator] = None) -> None:
    ...
    def tell(self, ys: np.ndarray, xs: Optional[np.ndarray] = None) -> None:
        if not xs is None:
            for p in range(self.popsize):
    ...
    def _next_x(self, p: int) -> np.ndarray:
    ...
    def _is_dominated(self, y: ArrayLike, p: int) -> bool:
...
def _filter(x: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ym = np.amax(y,axis=1)
...
def objranks(objs: np.ndarray) -> np.ndarray:
...
def ranks(cons: np.ndarray, feasible: np.ndarray, eps: float | np.ndarray) -> np.ndarray:
...
def get_valid(xs: np.ndarray, ys: np.ndarray, nobj: int) -> Tuple[np.ndarray, np.ndarray]:
    valid = (ys.T[nobj:].T <= 0).all(axis=1)
    return xs[valid], ys[valid]

def pareto_sort(x0: np.ndarray, y0: np.ndarray, nobj: int, ncon: int) -> Tuple[np.ndarray, np.ndarray]:
...
def pareto_domination(ys: np.ndarray,
                      nobj: int,
                      ncon: int,
                      last_ycon: Optional[np.ndarray] = None,
                      last_eps: float | np.ndarray = 0) -> Tuple[np.ndarray, Optional[np.ndarray], float | np.ndarray]:
    if ncon == 0:
...
def pareto_levels(ys: np.ndarray) -> np.ndarray:
    popn = len(ys)
...
def crowd_dist(y: np.ndarray) -> np.ndarray: # crowd distance for 1st objective
    n = len(y)
...
def feasible(xs: np.ndarray, ys: np.ndarray, ncon: int, eps: float = 1E-2) -> Tuple[np.ndarray, np.ndarray]:
...
def is_feasible(y: ArrayLike, nobj: int, eps: float = 1E-2) -> bool:
    ncon = len(y) - nobj
    if ncon == 0:
        return True
    else:
