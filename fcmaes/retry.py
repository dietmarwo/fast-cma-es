...
def plot3(front: ArrayLike, fname: str, label: Optional[str] =r'$\chi$',
         xlabel: Optional[str] = r'$f_1$', ylabel: Optional[str] = r'$f_2$',
         zlabel: Optional[str] = r'$f_3$') -> None:
    import matplotlib.pyplot as pl
    fig = pl.figure()
...
class Store(object):
    ...
    def replace(self, i: int, y: float, xs: ArrayLike) -> None:
        self.set_y(i, y)
        self.set_x(i, xs)
    ...
    def add_result(self, y: float, x: ArrayLike, evals: int, limit: float = np.inf) -> None:
    ...
    def create_xs_view(self) -> None: # needs to be called in the target process
    ...
    def incr_count_evals(self, evals: int) -> None:
...
def _convertBounds(bounds: Bounds | ArrayLike) -> Tuple[np.ndarray, np.ndarray]:
    if bounds is None:
