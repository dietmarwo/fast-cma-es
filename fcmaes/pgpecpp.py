...
"""Eigen based implementation of PGPE."""
...
from fcmaes._fcmaes_ext import PGPE as _PGPE
from fcmaes._fcmaes_ext import optimize_pgpe
...
def _prepare_sigma(dim: int, input_sigma: SigmaArg) -> np.ndarray:
    if callable(input_sigma):
        input_sigma = input_sigma()
    if np.ndim(input_sigma) == 0:
        input_sigma = np.full(dim, float(input_sigma), dtype=np.float64)
    return np.ascontiguousarray(input_sigma, dtype=np.float64)
...
def minimize(
    ...
    """Minimization using the nanobind-backed C++ PGPE implementation."""
    ...
    sigma = _prepare_sigma(dim, input_sigma)
    if lower is None:
...
class PGPE_C:
    def __init__(
        ...
        """Ask/tell PGPE state backed by nanobind."""
