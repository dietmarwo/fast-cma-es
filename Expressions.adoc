...
Each base algorithms has its own individually assigned evaluation budget - 50000 evaluations in the example above. With the coordinated parallel retry (`from fcmaes import advretry`) these budgets
...
* fcmaes predefined sequences: `de_cma, da_cma`, where `de_cma`, the sequence CMA-ES -> DE is the default algorithm used for the parallel retry.  
...
* Pagmo single objective algorithms: requires `pip install pygmo`
