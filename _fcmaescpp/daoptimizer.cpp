...
static vec logv(vec v) {
	return v.unaryExpr([](double x) { return log(x);});
}

static vec expv(vec v) {
	return  v.unaryExpr([](double x) { return exp(x);});
}
...
public:
	...
	VisitingDistribution(int dim, double visiting_param_, pcg64* rs_) {
		...
		double factor2 = exp((4.0 - _visiting_param) * log(
				_visiting_param - 1.0));
		double factor3 = exp((2.0 - _visiting_param) * log(2.0)
							/ (_visiting_param - 1.0));
		_factor4_p = sqrt(M_PI) * factor2 / (factor3 * (
				3.0 - _visiting_param));

		double factor5 = 1.0 / (_visiting_param - 1.0) - 0.5;
		double d1 = 2.0 - factor5;
		_factor6 = M_PI * (1.0 - factor5) / sin(
				M_PI * (1.0 - factor5)) / exp(lgamma(d1));
	}
	...
	vec visit_fn(double temperature, int dim) {
		...
		// sigmax
		x = x * exp(-(_visiting_param - 1.0) * log(
			_factor6 / factor4) / (3.0 - _visiting_param));

		vec den = expv(logv(y.cwiseAbs() * (_visiting_param - 1.0)) / (3.0 - _visiting_param));
...
public:
    ...
    void run(int step, double temperature) {
        ...
        not_improved_idx += 1;
        for (unsigned int j = 0; j < (state->current_location).size() * 2; j++) {
            if (j == 0)
    ...
    void local_search() {
        ...
        if (K < 90 * state->current_location.size()) {
            double pls = exp(K * (state->ebest - state->current_energy) / temperature_step);
            if (pls >= distr_01(*rs))
                do_ls = true;
        }
...
double minimize(Fittness* fun, vec& x0, long seed, 
                bool use_local_search, vec& X) {
...
extern "C" {
    double* optimizeDA_C(long runid, callback_type func, int dim, int seed,
        ...
        try {
        	...
        	double bestY = minimize(&fitfun, enc, seed, 
        	                        use_local_search, X);
