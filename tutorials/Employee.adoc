...
You search for the final challenge? Want to know how well your scheduling optimizer scales? For you we prepared 
a crazy hard task: More employees, more skills and a roster length of 70 days. Here are all modifications
in https://github.com/optapy/optapy-quickstarts/blob/stable/employee-scheduling/services.py[services.py] to generate the task:

----
FIRST_NAMES = ["Amy", "Beth", "Chad", "Dan", "Elsa", "Flo", "Gus", "Hugo", "Ivy", "Jay", "Carl", "Joy", "Marie", "Love", "Brad"]
LAST_NAMES = ["Cole", "Fox", "Green", "Jones", "King", "Li", "Poe", "Rye", "Smith", "Watt", "Sagan", "Field", "Curie", "Work", "Pitt"]
REQUIRED_SKILLS = ["Doctor", "Nurse"]
OPTIONAL_SKILLS = ["Anaesthetics", "Surgery", "Radiology"]
LOCATIONS = ["Ambulatory care", "Critical care", "Pediatric care"]
...
    INITIAL_ROSTER_LENGTH_IN_DAYS = 70
...
    for i in range(16):
        skills = pick_subset(OPTIONAL_SKILLS, random, 1, 4, 6)
----

You can load this problem using `p = problem('data/sched4.json')`

This means now the optimization problem has 630 decision variables. Even BiteOpt struggles to find a valid solution fulfilling
all constraints:

image::schedule.bite.2000k_bite cpp.png[]

With dimension = 630 both the optimizer overhead and fitness evaluation time increase. Evaluation rate drops to a meager
220.000 evals/sec, so that we only could perform 64 million evaluations in the given time. But finally we see no constraint violation
and:

----
solution = [12, 0, 1, 3, 4, 8, 7, 6, 9, 7, 3, 4, 11, 10, 6, 8, 0, 1, 6, 5, 13, 8, 9, 14, 0, 10, 1, 2, 7, 8, 6, 4, 12, 1, 11, 9, 10, 2, 7, 8, 15, 11, 13, 5, 3, 0, 12, 13, 6, 8, 4, 5, 10, 3, 6, 4, 11, 8, 12, 0, 10, 5, 2, 2, 15, 1, 3, 4, 13, 8, 10, 11, 15, 3, 13, 5, 0, 11, 4, 10, 8, 5, 0, 13, 9, 8, 1, 14, 15, 10, 8, 11, 12, 5, 10, 7, 3, 6, 1, 10, 4, 2, 12, 5, 6, 9, 11, 13, 8, 11, 9, 13, 3, 0, 2, 10, 1, 13, 4, 11, 14, 6, 12, 7, 9, 8, 15, 5, 14, 3, 12, 1, 8, 0, 9, 4, 7, 13, 1, 3, 15, 0, 14, 6, 9, 14, 12, 8, 5, 11, 1, 4, 3, 0, 8, 14, 7, 6, 13, 5, 2, 10, 5, 8, 15, 13, 11, 2, 1, 7, 14, 10, 8, 3, 4, 14, 0, 2, 5, 11, 11, 14, 5, 15, 6, 10, 7, 0, 3, 3, 15, 5, 8, 12, 7, 13, 2, 9, 8, 0, 3, 2, 11, 13, 15, 7, 9, 0, 5, 2, 6, 3, 8, 13, 12, 14, 8, 9, 15, 11, 12, 10, 6, 14, 0, 0, 9, 2, 3, 8, 14, 5, 10, 11, 6, 4, 9, 1, 5, 0, 10, 12, 2, 6, 13, 3, 7, 2, 8, 9, 5, 4, 0, 7, 1, 2, 10, 8, 5, 3, 14, 3, 7, 14, 5, 2, 0, 10, 1, 11, 11, 10, 9, 12, 13, 8, 3, 4, 14, 6, 8, 5, 1, 9, 4, 10, 15, 0, 3, 4, 12, 8, 9, 15, 6, 10, 11, 3, 12, 10, 8, 7, 14, 11, 1, 9, 13, 6, 11, 0, 4, 8, 3, 2, 5, 12, 15, 9, 0, 10, 4, 7, 2, 6, 3, 10, 13, 8, 9, 11, 1, 15, 2, 9, 2, 7, 0, 6, 5, 4, 8, 1, 5, 8, 3, 14, 15, 11, 1, 2, 7, 13, 14, 12, 11, 1, 5, 8, 3, 15, 12, 3, 9, 15, 7, 1, 14, 4, 0, 5, 13, 3, 9, 2, 8, 15, 10, 14, 4, 8, 14, 3, 2, 11, 0, 5, 13, 8, 3, 4, 14, 11, 10, 2, 9, 6, 6, 9, 8, 7, 15, 2, 5, 11, 12, 7, 2, 13, 5, 3, 10, 4, 1, 11, 10, 7, 8, 6, 3, 14, 15, 2, 4, 8, 6, 13, 10, 5, 12, 14, 3, 2, 7, 5, 12, 11, 15, 2, 8, 6, 3, 10, 5, 3, 14, 6, 12, 9, 15, 1, 9, 13, 11, 8, 12, 14, 6, 3, 7, 4, 11, 3, 5, 9, 14, 2, 15, 7, 1, 2, 3, 12, 15, 11, 14, 4, 13, 0, 6, 8, 5, 14, 15, 4, 9, 3, 8, 1, 10, 2, 3, 6, 14, 12, 9, 7, 10, 5, 8, 14, 11, 12, 2, 6, 10, 7, 9, 13, 0, 5, 15, 8, 4, 14, 8, 15, 2, 5, 12, 6, 9, 10, 12, 3, 14, 11, 5, 8, 7, 10, 1, 4, 12, 2, 3, 6, 8, 9, 5, 0, 8, 13, 4, 10, 3, 7, 11, 14, 1, 7, 13, 9, 15, 11, 10, 5, 0, 8, 1, 6, 3, 4, 7, 5, 14, 10, 0, 11, 13, 3, 14, 1, 10, 5, 2, 8, 10, 2, 14, 9, 4, 12, 5, 0, 8, 13, 6, 15, 8, 14, 5, 9, 2, 0, 5, 15, 6, 10, 8, 1, 3, 0, 11, 8, 1, 3, 5, 15, 7, 14, 0, 11, 14, 6, 10, 5, 11, 2, 3, 9, 15, 4, 15, 3, 1, 14, 7, 8, 10, 5]
28 desired shifts
shifts per employee [35, 33, 40, 50, 34, 49, 37, 34, 57, 38, 44, 41, 31, 31, 42, 34]
min shifts per employee 31
mean shifts per employee 39.375
std shifts per employee 7.253232038202004
----

For multi-objective optimization we now configure `popsize = 4096, max_evaluations = 10000000`.

[source,python]
----
        xs, ys = modecpp.retry(mode.wrapper(self.fitness_mo, 2), 
                     2, 0, self.bounds, popsize = 4096, max_evaluations = 10000000, 
                     nsga_update=True, num_retries = 32, workers=32)
----

Because of the larger optimizer overhead the evaluation rate drops even further to about 120.000 evaluations/sec. 
Computation takes more than 70 minutes. We get a 2 element pareto front, but with an interesting solution. 

----
solution = [14, 5, 0, 3, 12, 15, 6, 7, 1, 7, 8, 4, 1, 0, 6, 13, 5, 11, 13, 5, 15, 3, 2, 4, 1, 10, 7, 6, 11, 2, 14, 4, 9, 1, 7, 12, 0, 8, 6, 10, 14, 2, 13, 4, 5, 4, 6, 13, 1, 3, 12, 10, 8, 0, 12, 4, 7, 3, 6, 10, 0, 14, 8, 3, 6, 10, 0, 14, 15, 8, 1, 2, 6, 4, 11, 10, 9, 8, 5, 3, 15, 15, 3, 13, 6, 1, 11, 0, 14, 10, 11, 15, 13, 3, 9, 7, 8, 14, 1, 0, 14, 3, 12, 8, 7, 6, 2, 11, 10, 11, 12, 15, 3, 5, 2, 8, 7, 14, 15, 1, 5, 7, 6, 13, 9, 11, 11, 0, 4, 9, 15, 13, 8, 5, 1, 4, 7, 15, 6, 10, 11, 3, 13, 1, 1, 10, 7, 9, 5, 2, 12, 4, 0, 9, 3, 4, 12, 7