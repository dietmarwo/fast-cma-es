...
class BaseProblem:
    ...
    def __init__(self):                
        ...
        self.ints = [v != 'cont' for v in problem.vartype()]
...
After the fcmaes DE algorithm got an "mixed integer" upgrade, we executed the experiment again. Since almost all ESP variables
...
After the fcmaes DE algorithm got an "mixed integer" upgrade, we executed the experiment again. Since many HPO variables
