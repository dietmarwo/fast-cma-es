...
class DockerHospitalBenchmarkProblem(BaseProblem):
    ...
    def __init__(self, name, d, lbs, ubs, vartype, direction, errval):
        ...
        # to monitor progress we need some variables shared across python processes
        ...
        if y < self.best_y.value:
            ...
            logger().info("evals = {0}: time = {1:.1f} y = {2:.5f} x= {3:s}"
                          .format(self.evals.value, dtime(self.t0), y, 
                                  str([round(xi,5) for xi in xs])
                    ))
        return y
                
    def optimize(self, popsize = 16, max_evaluations = 3000):
        self.bestY = 1E99
        self.bestX = []
        #return cmaes.minimize(self.fun, 
...
Is four hours too much for a problem of this complexity? I doubt the hospital manager has a problem with that.  

How can the hospital manager decide if he should use something more fancy, for instance a surrogate model - 
or any other innovative method?

When we search for published results for the hospital management problem we often find
https://www.youtube.com/watch?v=Riio1eKOSKg&t=711s[Jan Hendrik Schön]-style results: 
Reporting relative but no absolute optimization results. https://en.wikipedia.org/wiki/Sch%C3%B6n_scandal[Schön] 
proved that you can almost get a nobel price publishing this way, 
but it makes it difficult to evaluate the tested methods compared to other ones. 
...
200 evaluations. Question is, if this helps the hospital manager to decide if it is worth
investing in these methods - which means to hire a development team since the used code is not yet published on github. 
How do these methods scale when investing more evaluations? 
And even more important: How do they scale when investing more time considering parallelization?
All we know is that using the surrogate model based method you get mediocre solutions quite fast.
