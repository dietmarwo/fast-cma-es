...
about 163.000 transfers into a selection of about 380 transfers. 

Converting an approximated transfer into a "real one" accepted by the GTOC11 validation costs a lot
of CPU resources. The scheduling algorithm can be used to minimize this effort by reducing the
number of transfers it needs to be applied to:

- Apply the scheduling algorithm for a limited time using BiteOpt - which converges faster
than the SBM algorithm - to determine which transfers need to be converted. 
...
- Finally apply the scheduling algorithm using SMB - which converges slower but finds better results if 
given enough time. 
