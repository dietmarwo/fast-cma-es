...
of the best solution found over time both for using BiteOpt and the smart boundary management (SBM). 
...
smart boundary management (SBM) can be used. 
...
about 163.000 transfers into a selection of about 390 transfers. 
...
- Finally apply the scheduling algorithm using SBM - which converges slower but finds better results if 
given enough time. 
