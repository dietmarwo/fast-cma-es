...
The multi-objective optimization method `retry_modecpp` calls `modecpp.minimize` in parallel, 
but the optimization itself is single threaded (`workers = 1`). The random generators need to be
initialized using `sg.spawn(workers)` to preserve diversification of the parallel optimization runs. 
...
def retry_modecpp(fit, retry_num = 64, popsize = 48, max_eval = 5000000, 
                     nsga_update = False, workers=mp.cpu_count()):
    store = mode.store(len(fit.bounds.lb), fit.nobj + fit.ncon, retry_num*popsize*2)
    i = 0
    while i < retry_num:
        sg = SeedSequence()
        rgs = [Generator(MT19937(s)) for s in sg.spawn(workers)]
        proc=[Process(target=run_modecpp,
...
The NSGA-II population update improves faster, but finally the DE population update sometimes delivers better
results. One DE-update-run delivered a score of 7037 using 60 trajectories after about 1000 seconds.   
