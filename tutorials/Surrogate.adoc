...
def fitness(x):
    try:
        ...
        y = np.empty(nobj)
        c = np.empty(ncon)
        # the integer x values are converted into the discrete values from the table
...
therefore the `return y[:2], c * -1`. Now we can define the problem class:
...
class madzda_problem(object):
    ...
    def eval(self, x, is_multi):
        y, constr = fitness(x)
        constr = np.maximum(constr, 0) # we are only interested in constraint violations       
        fval = np.array(list(y) + list(constr))              
        ys = self.eval_single(x, fval, is_multi)
        self.n_evals.value += 1                               
        if ys < self.best_y.value: # monitor progress
            self.best_y.value = ys
            print(str(dtime(self.time_0)) + ' ' + 
                str(self.n_evals.value) + ' ' + 
                str(round(self.n_evals.value/(1E-10 + dtime(self.time_0)),0)) + ' ' + 
                str(self.best_y.value) + ' ' +
                str(list(y)) + ' ' + str(list(constr)))                  
        return y, ys, fval, constr
        
    def fun(self, x): # multi objective value
        y, ys, fval, constr = self.eval(x, True)
        return fval
        
    def sfun(self, x): # single objective value, constraint via weighted sum
        y, ys, fval, constr = self.eval(x, False)
        return ys
    ...
    def eval_single(self, x, fval, is_multi):
        # constraints get a higher weight than the objectives
        weights = np.array([1, 1] + [3]*ncon) if is_multi else np.array([1, 0] + [3]*ncon)
...
---- 
       for i in range(20):
            ...
            ret = de.minimize(problem.sfun, dim, problem.bounds,
...
---- 
        problem = madzda_problem()
        x, y = modecpp.minimize(problem.sfun_m, 1, 0, problem.bounds, popsize = 64, 
...
---- 
        problem = madzda_problem()
        x, y = modecpp.minimize(problem.sfun_c, 1, problem.ncon, problem.bounds, popsize = 64, 
            max_evaluations = 1000000, pareto_update=1, log_period=100000,
            nsga_update=False, workers=32, ints = np.array([True]*dim))
----
...
[source,python]
---- 
       x, y = modecpp.minimize(problem.fun, problem.nobj, problem.ncon, problem.bounds, popsize=256, 
