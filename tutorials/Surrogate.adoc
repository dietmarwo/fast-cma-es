...
could be tested (50000 using population size 512). The results are disappointing, each run took about 2 hours:

image::AGO512.50k.png[]

- CTAEA seems not to be able to find feasible solutions. Very soon it shows very good objective values
(2.1, 74) but there are severe constraint violations.
...
car design problem, we miss about 15-30 common part thicknesses.  

Here are 6 runs with 500000 generations using population size 512 each taking about 2 hours:
 
image::NSGAII512.500k.png[]

Next are 9 runs with 400000 generations using population size 768 delivering slightly better results:

image::NSGAII768.400k.png[]

These results a quite far away from what we saw above what fcmaes MODE (modecpp) can do using only 1600 seconds. 
