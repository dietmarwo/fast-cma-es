...
=== Update: cmaes version 1.4.3 now supports CR-FM-NES parallel function evaluation 

cmaes version 1.4.2 doesn't support parallel function evaluation for CR-FM-NES. For 1.4.3 this is now implemented (do `pip install fcmaes --upgrade`). Works both for the Python and the C++ version. This changes the picture completely: With 8000 evaluations and population
size 16 (which makes CR-FM-NES more reliable, 15 out of 16 runs resulted in perfect energy=7.0), we get:
