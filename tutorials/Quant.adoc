...
We see 25% CPU load corresponding to 8 cores on our 16 core CPU. How much did we gain by parallelization?
...
Again parallelization hurt the performance. And we observe that the optimization results differ (we got lucky here)
...
so that we could run our experiments in parallel utilizing multiple cloud nodes. 

In winter time we observe an advantage of qiskits parallelization: CPU temperature raised by 10 degrees Celsius
