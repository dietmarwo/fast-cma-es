...
https://www.youtube.com/watch?v=2Eswqed8agg[Quantum programming], https://qiskit.org/textbook[textbook] and http://mmrc.amss.cas.cn/tlb/201702/W020170224608150507023.pdf[introduction] provide 
...
amplitude amplification based on phase resonances which single out the desired bit combination 
...
- aer_simulator, qasm_simulator and aer_simulator_statevector all don't scale at all, neither with multi-threading nor by using a GPU. 
...
- For aer_simulator_density_matrix the picture changes if we reduce the number of qubits to 8:
  Now single threaded execution is faster than multi threading and GPU usage looses its advantage.
