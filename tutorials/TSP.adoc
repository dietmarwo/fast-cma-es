...
random matrices with objective function: f(x) = (x − x^∗ )^T A(x − x^∗) applied to binary input values. 
Here IDONE indeed uses far less function evaluations as all standard algorithms we tried. But what happens if
