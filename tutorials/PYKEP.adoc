...
PYKEP provides a set of https://github.com/esa/pykep/tree/master/pykep/trajopt/gym[reference problems] for testing optimization methods. Many of them replace the older GTOP problems.
These newer implementations have several practical advantages:
...
Another limitation is the lack of reference solutions. The problems are implemented as PAGMO-UDPs, see https://esa.github.io/pagmo2/docs/cpp/problem.html[Problem/UDP], so solving them with PYGMO/PAGMO is a natural starting point. But there is little guidance on how to configure a https://esa.github.io/pagmo2/docs/cpp/archipelago.html[PAGMO-Archipelago] well for these cases. https://esa.github.io/pykep/examples/ex13.html[Example 13] shows single-threaded runs, but not a parallel optimization workflow.

Most of these problems are single-objective and unconstrained, or can be made unconstrained with https://esa.github.io/pagmo2/docs/cpp/problems/unconstrain.html[unconstrain]. That makes them a good fit for `fcmaes`. They are also a useful way to show what coordinated retry can do on real trajectory optimization tasks.

=== Applying fcmaes coordinated retry to PYKEP trajopt/gym
See https://github.com/esa/pykep/tree/master/pykep/trajopt/gym[trajopt/gym] for the full list of problems.

==== Tandem

https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_tandem.py[Tandem] models a rendezvous mission to Saturn as an MGA-1DSM transfer. The mission parameters are inspired by the TandEM mission. A launcher model, Atlas 501, is included, so the main objective is the final mass delivered at Saturn.
...
==== Messenger
https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_messenger.py[Messenger] models a mission to Mercury as an MGA-1DSM transfer. The selected fly-by sequence, `E-VVMeMeMe-Me`, and the other parameters are inspired by the Messenger mission.
...
This matters when you are screening planet sequences before a final optimization pass. In that phase, regular retry (`fcmaes.retry`) can be faster. In one hour you can check about 150 sequences. Messenger is a reminder that this does not work equally well for every problem. If many resonances are present, more care is needed. You can also wrap other algorithms from https://esa.github.io/pagmo2/docs/cpp/cpp_docs.html#implemented-algorithms[PAGMO] for use with `fcmaes.retry`, but some of them are hard to configure well.
----
...
==== Cassini2
https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_cassini2.py[Cassini2] is an MGA-1DSM mission to reach Saturn and be captured by its gravity. The fly-by sequence is Earth, Venus, Venus, Earth, Jupiter, Saturn, the same sequence used by the Cassini spacecraft. A score `< 730` is usually reached after about three minutes. Unlike Juice, this problem does need coordinated retry.
----
...
https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_eve_mga1dsm.py[EVE] is an MGA-1DSM mission to Venus and back to Earth. A good score is usually found in a few seconds.
----
...
https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_emNimp_udp.py[Earth-Mars 5 impulses] is a 5-impulse deep-space maneuver mission to reach Mars. A good score is reached after a few seconds.
----
...
https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_emNimp_udp.py[Earth-Mars 7 impulses] is a 7-impulse deep-space maneuver mission to reach Mars. A good score is reached after a few seconds.
----
...
We can also study a multi-objective variant of https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_juice.py[Juice].
...
----
See https://esa.github.io/pagmo2/docs/cpp/algorithms/moead.html[moead], https://esa.github.io/pagmo2/docs/cpp/algorithms/nsga2.html[nsga2], and https://esa.github.io/pagmo2/docs/cpp/algorithms/maco.html[maco].
...
- It is not clear how to configure Pygmo's parallel execution mechanism, `archipelago`, well for this problem.
...
Another option is the https://www.researchgate.net/publication/225485886_The_weighted_sum_method_for_multi-objective_optimization_New_insights[weighted sum] approach combined with `fcmaes` coordinated retry. Instead of using one fixed weight, we sweep over different weights and generate good solutions along the Pareto front. That gives us a practical benchmark for the dedicated MOO algorithms.
...
Can MOO optimizers like https://esa.github.io/pagmo2/docs/cpp/algorithms/moead.html[moead], https://esa.github.io/pagmo2/docs/cpp/algorithms/nsga2.html[nsga2], and https://esa.github.io/pagmo2/docs/cpp/algorithms/maco.html[maco] find similar results? How should we configure them, and what is the right `archipelago` topology for parallel execution?

=== Tandem Planet Sequence
In the discussion of the https://github.com/esa/pykep/blob/master/pykep/trajopt/gym/_tandem.py[Tandem] problem above, we chose the 6th planet sequence, `[earth, venus, earth, earth, saturn]`, because we suspected it was the best one. Now we can test that assumption. Instead of giving every sequence a full optimization budget up front, we use an iterative elimination approach. Each round applies parallel retry to all remaining sequences for a limited number of retries. We then remove the worst ones and continue with the rest. This concentrates more compute time on the better candidates.
...
Sequence 6, `[earth, venus, earth, earth, saturn]`, the one we used before, wins as expected. The https://www.esa.int/gsp/ACT/projects/gtop/tandem_unc[GTOP result] is worse, but the problems are not identical. At https://github.com/esa/pykep/blob/42ccdfb812d77ecb085a58419a10e7e86787d38c/pykep/trajopt/gym/_tandem.py#L116[L116], the GTOP Tandem problem uses the "wrong direction" for `Vinfz`, and this is corrected in pykep. A different Lambert solver is also used.
