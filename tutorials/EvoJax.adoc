...
https://cloud.google.com/blog/topics/developers-practitioners/evojax-bringing-power-neuroevolution-solve-your-problems[evojax intro]EvoJax
...
- Other algorithms may perform well on a CPU, specially if implemented in {cpp}, and even improve the convergence. 

But fcmaes provides one algorithms specialized for high dimensional problems: CR-FM-NES, which we will compare with the other 
