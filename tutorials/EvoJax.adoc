...
Only PGPE and FCRFMC/CRFMNES are successful. This is the only benchmark where FCRFMC is better than CRFMNES - probably because
...
- FCRFMC/CRFMNES shows the highest improvement rate for increasing optimization budget and may be an alternative for even more complex tasks. Note that FCRFMC, 
...
- CRFMNES, the 'jaxified' Python implementation of CR-FM-NES has no significant wall time disadvantage compared to the {cpp}-version FCRFMC, sometimes it is even faster. Mostly the reduced accuracy doesn't harm the convergence. 
