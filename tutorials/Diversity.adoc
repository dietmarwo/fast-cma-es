...
- Mixing MAP-elites with an CR-FM-NES improvement emitter in most cases works very well and should be tried first.
...
- The default configurations provided by fcmaes work well, but the population size can be
adapted dependent on the number of fitness evaluations our budged allows. Higher population size also reduces the overhead for the tesselation because it increases its batch size. Compared with MO-algorithms the population size is less relevant.
...
borrowed from NSGA-II works better. You may try it to further improve existing QD-archives.
- Improvement emitters ofter are initialized with niche-elites. We found no
