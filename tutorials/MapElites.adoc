...
are delegated to a CPU/TPU. But as the benchmarks results in 
https://github.com/dietmarwo/fast-cma-es/blob/master/tutorials/EvoJax.adoc[EvoJax.adoc] show: For these high dimensional 
problems CMA-ES is not the best choice anyway because its slow convergence. For MAP-Elites which typically is
