...
For comparison; The original non-numba simulation needs 277 simulations / sec 
...
much smaller compared to the cost of a function evaluation. 
...
Parallel optimization produces much more detailed picture, since because of the 
improved parallelization-scaling we can compute a much bigger solution.

We see that we should not invest more than 800 for the hand-washing campaign, since
...
budged further reduces the final infection rate significantly. But if 
our overall budged is less than 800, all the money should go to the hand-washing campaign. 
...
Again we get a much more detailed picture using parallel optimization, since, because of the 
improved parallelization-scaling we can compute a much bigger solution this way. Note that this
