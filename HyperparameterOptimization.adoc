...
applicable. Still these are completely ignored - beside some research applying Differential Evolution.  
...
using parallel function evaluation compared to the XGBRegressor internal parallelism. The number of evaluated functions 
in a given time more than doubled for CMA-ES. But memory consumption is higher for parallel function evaluation, since
multiple parallel instances of the XGBRegresso are maintained. The results are discouraging
for Bayesian Optimization and Parzen Tree Search, they performed significantly worse than the other methods. 
...
real value boundaries the upper bound should be `6` so that max_depth=5 has also
...
#ret = decpp.minimize(obj_f, 8, bounds, popsize=13, max_evaluations = 20000)
----
